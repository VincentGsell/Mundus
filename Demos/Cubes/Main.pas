...
type
  ...
  private
    ...
    //Utils
    procedure AddNewCube(Position : TFloat3; Shader : TShaderClass);
  public
...
procedure TForm1.AddNewCube(Position: TFloat3; Shader : TShaderClass);
var sampleCube : TCube;
begin
  sampleCube := TCube.Create;
  sampleCube.Position := Position;
  sampleCube.Rotation := Float3(0, 0, 45);
  sampleCube.Shader := Shader;
  FSoftwareRenderer.MeshList.Add(sampleCube);
end;
...
begin
  ...
  FTexture2.LoadFromFile('..\..\Texture.bmp');

  FSoftwareRenderer := TMundusRenderer.Create();
  AddNewCube(Float3(0, 0, 300),TTextureShader);
  AddNewCube(Float3(-70, 0, 300),TTextureShader);
  AddNewCube(Float3(70, 0, 300),TTextureShader);
...
var FCube : TCube;
begin
  FCube := TCube(FSoftwareRenderer.MeshList[0]);
...
begin
  ...
  l := rSpeed;
  for FCube in FSoftwareRenderer.MeshList do
  begin
    l := l + rSpeed;
    FCube.Rotation := Float3(FCube.Rotation.X + l, FCube.Rotation.Y, FCube.Rotation.Z);
  end;
end;
...
begin
  ...
  if FSoftwareRenderer.MeshList.IndexOf(AMesh) = 0 then //lol
    ABuffer.Texture[0] := FTexture2; //  ABuffer.Texture[ABuffer.Texture.Bind('Tex0')] := FTexture2; //Does not work. should be.
